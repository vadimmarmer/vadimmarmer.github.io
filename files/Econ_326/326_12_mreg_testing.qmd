...
- Under our assumptions, conditional on $\mathbf{X}$, the OLS estimator $\hat{\beta}_{j}$ satisfies $\hat{\beta}_{j}\sim N\left(\beta_{j},\Var{\hat{\beta}_{j} \mid \mathbf{X}}\right),$ where $\Var{\hat{\beta}_{j} \mid \mathbf{X}}=\sigma^{2}/\sum_{i=1}^{n}\tilde{X}_{j,i}^{2}$ (see Lecture 11).

- Therefore, $\left(\hat{\beta}_{j}-\beta_{j}\right)/\sqrt{\Var{\hat{\beta}_{j} \mid \mathbf{X}}}\sim N\left(0,1\right).$

- The conditional variance $\Var{\hat{\beta}_{j} \mid \mathbf{X}}$ is unknown because $\sigma^{2}$ is unknown. The estimator for $\Var{\hat{\beta}_{j} \mid \mathbf{X}}$ is
...
- **P-value:** Find $t_{n-k-1,1-\tau}$ such that $\left\vert T\right\vert=t_{n-k-1,1-\tau}.$ The p-value $=\tau\times 2.$
...
- **Example 1:** Consider the model
  ...
  - In this case: $c_{0}=0, c_{1}=1, c_{2}=1, r=1.$
...
- **Example 2:** Consider the model
  ...
  $$\begin{aligned}
  \ln\left(\mathit{Wage}_{i}\right) &= \beta_{0}+\beta_{1}\mathit{Experience}_{i}+\beta_{2}\mathit{PrevExperience}_{i} \\
  &\quad +\beta_{3}X_{3,i}+\ldots+\beta_{k}X_{k,i}+U_{i}.
  \end{aligned}$$

  - We want to test that $\mathit{Experience}$ and $\mathit{PrevExperience}$ have the same effect on wage: $H_{0}:\beta_{1}=\beta_{2}$ or $H_{0}:\beta_{1}-\beta_{2}=0.$

  - In this case: $c_{0}=0, c_{1}=1, c_{2}=-1, c_{3}=\ldots=c_{k}=0, r=0.$
...
- The estimated variance:
  ...
  $$
  \Vhat{\hat{\beta}_{1}+\hat{\beta}_{2}}=\Vhat{\hat{\beta}_{1}}+\Vhat{\hat{\beta}_{2}}+2\widehat{\mathrm{Cov}}\left(\hat{\beta}_{1},\hat{\beta}_{2}\right).
  $$

  - $\Vhat{\hat{\beta}_{1}}$ and $\Vhat{\hat{\beta}_{2}}$ can be computed from the corresponding standard errors reported by R.
...
- The 95% confidence interval for the coefficient on $\ln K$ in the transformed model does not include 1, so we reject $H_{0}.$

- In the original equation, $\hat{\beta}_{1}+\hat{\beta}_{2}$ equals the coefficient on $\ln K$ in the reparameterized regression, and $\se{\hat{\beta}_{1}+\hat{\beta}_{2}}$ equals its standard error.
...
- By default, `linearHypothesis()` reports an F-statistic. For a single restriction, $F = T^{2}$ and the p-values are identical to the two-sided t-test.
