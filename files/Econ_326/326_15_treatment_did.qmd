---
title: "Lecture 15: Causal inference"
...
format:
  html:
    output-file: 326_15_treatment_did.html
    toc: true
    toc-depth: 3
    toc-location: right
  ...
  pdf:
    output-file: 326_15_treatment_did.pdf
    pdf-engine: xelatex
    ...
    classoption: fleqn
  revealjs:
    output-file: 326_15_treatment_did_slides.html
    date: ""
    theme: solarized
...
- **Conditional mean independence assumption:**
  ...
  $$
  \E{U_i(0) \mid X_i, D_i} = 0 \quad \text{and} \quad \E{U_i(1) \mid X_i, D_i} = 0.
  $$
...
- The observed outcome $Y_i = D_i Y_i(1) + (1 - D_i) Y_i(0)$ can be written as a single regression. Expanding:
  ...
  \begin{align*}
  Y_i &= D_i \underbrace{({\color{blue}\alpha_1} + {\color{blue}\beta_1} X_i + {\color{blue}U_i(1)})}_{Y_i(1)} + (1 - D_i)\underbrace{({\color{red}\alpha_0} + {\color{red}\beta_0} X_i + {\color{red}U_i(0)})}_{Y_i(0)} \\
  &= {\color{red}\alpha_0}(1 - D_i) + {\color{blue}\alpha_1} D_i \\
  &\quad + {\color{red}\beta_0} X_i(1 - D_i) + {\color{blue}\beta_1} X_i D_i + (1 - D_i){\color{red}U_i(0)} + D_i {\color{blue}U_i(1)} \\
  &= {\color{red}\alpha_0} + ({\color{blue}\alpha_1} - {\color{red}\alpha_0})D_i \\
  &\quad + {\color{red}\beta_0} X_i + ({\color{blue}\beta_1} - {\color{red}\beta_0})X_i D_i + U_i,
  \end{align*}

  where $U_i = (1 - D_i){\color{red}U_i(0)} + D_i {\color{blue}U_i(1)}$.
...
- This is a regression of $Y_i$ on $D_i$, $X_i$, and the interaction $X_i D_i$:
  ...
  where $\gamma = {\color{blue}\alpha_1} - {\color{red}\alpha_0}$ and $\delta = {\color{blue}\beta_1} - {\color{red}\beta_0}$.

- The coefficient on $D_i$ is $\gamma = \alpha_1 - \alpha_0$, which is **not** the ATE (unless $\beta_1 = \beta_0$).

## The demeaning trick

- Recall that $\text{ATE} = (\alpha_1 - \alpha_0) + (\beta_1 - \beta_0)\E{X_i}$, but the coefficient on $D_i$ is only $\alpha_1 - \alpha_0$. To fix this, **add and subtract** $(\beta_1 - \beta_0)\E{X_i}D_i$:

  \begin{align*}
  Y_i &= \alpha_0 + (\alpha_1 - \alpha_0)D_i + \beta_0 X_i + (\beta_1 - \beta_0) X_i D_i + U_i \\
  &= \alpha_0 + \big[(\alpha_1 - \alpha_0) + {\color{blue}(\beta_1 - \beta_0)\E{X_i}}\big]D_i \\
  &\quad + \beta_0 X_i + (\beta_1 - \beta_0) X_i D_i {\color{red}{}- (\beta_1 - \beta_0)\E{X_i}D_i} + U_i \\
  &= \alpha_0 + \underbrace{\big[(\alpha_1 - \alpha_0) + {\color{blue}(\beta_1 - \beta_0)\E{X_i}}\big]}_{\text{ATE}} D_i \\
  &\quad + \beta_0 X_i + (\beta_1 - \beta_0){\color{red}(X_i - \E{X_i})}D_i + U_i.
  \end{align*}

- Defining $\tau = \text{ATE}$ and $\delta = \beta_1 - \beta_0$:

  $$
  Y_i = \alpha_0 + \tau D_i + \beta_0 X_i + \delta(X_i - \E{X_i})D_i + U_i.
  $$
...
- Comparing group averages $\E{Y_i \mid D_i = 1} - \E{Y_i \mid D_i = 0}$ does not generally yield the ATE because of **selection bias**: treated and control groups may differ in their baseline characteristics.
...
- With covariates, $\text{ATE} = (\alpha_1 - \alpha_0) + (\beta_1 - \beta_0)\E{X_i}$ depends on both intercept and slope differences.
