...
- For our model, conditional on $\mathbf{X}$:
  $$
  \begin{aligned}
  \hat{\beta}_{1} &\sim N\left( \beta _{1},\Var{\hat{\beta}_{1}} \right), \\
  \Var{\hat{\beta}_{1}} &=\frac{\sigma ^{2}}{\sum_{i=1}^{n}\left( X_{i}-\bar{X}\right) ^{2}}.
  \end{aligned}
  $$
...
- In choosing $c_{1-\alpha }$ we will be relying on the fact that given our assumptions and conditionally on $\mathbf{X}$:
  $$
  \hat{\beta}_{1}\sim N\left( \beta _{1},\Var{\hat{\beta}_{1}}\right) \text{ and }\Var{\hat{\beta}_{1}} =\frac{\sigma ^{2}}{\sum_{i=1}^{n}\left( X_{i}-\bar{X}\right) ^{2}}.
  $$

- Note that conditionally on $\mathbf{X}$:
  $$
  \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{\Var{\hat{\beta}_{1}} }}\sim N\left( 0,1\right) .
  $$
...
- **Suppose** (for a moment) that $\sigma ^{2}$ is known, and we can compute exactly the variance of $\hat{\beta}_{1}$ as $\Var{\hat{\beta}_{1}} =\sigma ^{2}/\sum_{i=1}^{n}\left( X_{i}-\bar{X}\right) ^{2}.$

- Consider the following CI:
  $$
  CI_{1-\alpha }=\left[ \hat{\beta}_{1}-z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} },\hat{\beta}_{1}+z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} }\right] .
  $$

- For example, if $1-\alpha =0.95 \Longleftrightarrow \alpha =0.05\Longleftrightarrow z_{1-\alpha/2}=z_{0.975}=1.96$, and
  $$
  CI_{0.95}=\left[ \hat{\beta}_{1}-1.96\sqrt{\Var{\hat{\beta}_{1}} },\hat{\beta}_{1}+1.96\sqrt{\Var{\hat{\beta}_{1}} }\right] .
  $$

## Validity of the infeasible CIs ($\sigma^2$ is known)

- We need to show that $P\left( \beta _{1}\in \left[ \hat{\beta}_{1}-z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} },\hat{\beta}_{1}+z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} }\right] \right)=1-\alpha .$

- Next,
  $$
  \begin{aligned}
  &\hat{\beta}_{1}-z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} }\leq \beta _{1}\leq \hat{\beta}_{1}+z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} } \\
  &\Longleftrightarrow -z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} }\leq \beta _{1}-\hat{\beta}_{1}\leq z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} } \\
  &\Longleftrightarrow -z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} }\leq \hat{\beta}_{1}-\beta _{1}\leq z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} } \\
  &\Longleftrightarrow -z_{1-\alpha /2}\leq \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{\Var{\hat{\beta}_{1}} }}\leq z_{1-\alpha /2}
  \end{aligned}
  $$
...
- We have that
  $$
  \begin{aligned}
  &\beta _{1}\in \left[ \hat{\beta}_{1}-z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} },\hat{\beta}_{1}+z_{1-\alpha /2}\sqrt{\Var{\hat{\beta}_{1}} }\right] \\
  &\Longleftrightarrow -z_{1-\alpha /2}\leq \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{\Var{\hat{\beta}_{1}} }}\leq z_{1-\alpha /2}.
  \end{aligned}
  $$

- Let $Z=\frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{\Var{\hat{\beta}_{1}} }}\sim N\left( 0,1\right)$.
  $$
  \begin{aligned}
  &P\left( -z_{1-\alpha /2}\leq \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{\Var{\hat{\beta}_{1}} }}\leq z_{1-\alpha /2}\right) \\
...
- We can replace $\sigma ^{2}$ by $s^{2}$, however, the result does not have a normal distribution any more:
  $$
  \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{\widehat{\mathrm{Var}}\left( \hat{\beta}_{1}\right) }}\sim t_{n-2},\text{ where }\widehat{\mathrm{Var}}\left( \hat{\beta}_{1}\right) =\frac{s^{2}}{\sum_{i=1}^{n}\left( X_{i}-\bar{X}\right) ^{2}}.
  $$
...
- We can now construct a feasible confidence interval with $1-\alpha$ coverage as:
  $$
  \begin{aligned}
  CI_{1-\alpha } &=\left[ \hat{\beta}_{1}-t_{n-2,1-\alpha /2}\sqrt{\widehat{\mathrm{Var}}\left( \hat{\beta}_{1}\right) },\hat{\beta}_{1}+t_{n-2,1-\alpha /2}\sqrt{\widehat{\mathrm{Var}}\left( \hat{\beta}_{1}\right) }\right], \\
