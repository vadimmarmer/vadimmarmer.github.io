...
- Our model:
  ...
  2. $E\left( U_{i}|X_{1},\ldots ,X_{n}\right) =0$ for all $i$'s.

  3. $E\left( U_{i}^{2}|X_{1},\ldots ,X_{n}\right) =\sigma ^{2}$ for all $i$'s.

  4. $E\left( U_{i}U_{j}|X_{1},\ldots ,X_{n}\right) =0$ for all $i\neq j$.
...
- For our model, conditional on $X$'s:
  $$
  \begin{aligned}
  \hat{\beta}_{1} &\sim N\left( \beta _{1},Var\left( \hat{\beta}_{1}\right) \right), \\
  Var\left( \hat{\beta}_{1}\right) &=\frac{\sigma ^{2}}{\sum_{i=1}^{n}\left( X_{i}-\bar{X}\right) ^{2}}.
  \end{aligned}
  $$
...
- The formal definition of CI: It is a **random interval** $CI_{1-\alpha}$ such that conditional on $X$'s,
  $$
  P\left( \beta _{1}\in CI_{1-\alpha }\right) =1-\alpha .
  $$
...
- In choosing $c_{1-\alpha }$ we will be relying on the fact that given our assumptions and conditionally on $X$'s:
  $$
  \hat{\beta}_{1}\sim N\left( \beta _{1},Var\left( \hat{\beta}_{1}\right)\right) \text{ and }Var\left( \hat{\beta}_{1}\right) =\frac{\sigma ^{2}}{\sum_{i=1}^{n}\left( X_{i}-\bar{X}\right) ^{2}}.
  $$

- Note that conditionally on $X$'s:
  $$
  \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{Var\left( \hat{\beta}_{1}\right) }}\sim N\left( 0,1\right) .
  $$
...
- **Suppose** (for a moment) that $\sigma ^{2}$ is known, and we can compute exactly the variance of $\hat{\beta}_{1}$ as $Var\left( \hat{\beta}_{1}\right) =\sigma ^{2}/\sum_{i=1}^{n}\left( X_{i}-\bar{X}\right) ^{2}.$

- Consider the following CI:
  $$
  CI_{1-\alpha }=\left[ \hat{\beta}_{1}-z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) },\hat{\beta}_{1}+z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) }\right] .
  $$

- For example, if $1-\alpha =0.95 \Longleftrightarrow \alpha =0.05\Longleftrightarrow z_{1-\alpha/2}=z_{0.975}=1.96$, and
  $$
  CI_{0.95}=\left[ \hat{\beta}_{1}-1.96\sqrt{Var\left( \hat{\beta}_{1}\right) },\hat{\beta}_{1}+1.96\sqrt{Var\left( \hat{\beta}_{1}\right) }\right] .
  $$

## Validity of the infeasible CIs ($\sigma^2$ is known)

- We need to show that $P\left( \beta _{1}\in \left[ \hat{\beta}_{1}-z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) },\hat{\beta}_{1}+z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) }\right] \right)=1-\alpha .$

- Next,
  $$
  \begin{aligned}
  &\hat{\beta}_{1}-z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) }\leq \beta _{1}\leq \hat{\beta}_{1}+z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) } \\
  &\Longleftrightarrow -z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) }\leq \beta _{1}-\hat{\beta}_{1}\leq z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) } \\
  &\Longleftrightarrow -z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) }\leq \hat{\beta}_{1}-\beta _{1}\leq z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) } \\
  &\Longleftrightarrow -z_{1-\alpha /2}\leq \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{Var\left( \hat{\beta}_{1}\right) }}\leq z_{1-\alpha /2}
  \end{aligned}
  $$
...
- We have that
  $$
  \begin{aligned}
  &\beta _{1}\in \left[ \hat{\beta}_{1}-z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) },\hat{\beta}_{1}+z_{1-\alpha /2}\sqrt{Var\left( \hat{\beta}_{1}\right) }\right] \\
  &\Longleftrightarrow -z_{1-\alpha /2}\leq \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{Var\left( \hat{\beta}_{1}\right) }}\leq z_{1-\alpha /2}.
  \end{aligned}
  $$

- Let $Z=\frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{Var\left( \hat{\beta}_{1}\right) }}\sim N\left( 0,1\right)$.
  $$
  \begin{aligned}
  &P\left( -z_{1-\alpha /2}\leq \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{Var\left( \hat{\beta}_{1}\right) }}\leq z_{1-\alpha /2}\right) \\
...
- We can replace $\sigma ^{2}$ by $s^{2}$, however, the result does not have a normal distribution any more:
  $$
  \frac{\hat{\beta}_{1}-\beta _{1}}{\sqrt{\widehat{Var}\left( \hat{\beta}_{1}\right) }}\sim t_{n-2},\text{ where }\widehat{Var}\left( \hat{\beta}_{1}\right) =\frac{s^{2}}{\sum_{i=1}^{n}\left( X_{i}-\bar{X}\right) ^{2}}.
  $$
...
- We can now construct a feasible confidence interval with $1-\alpha$ coverage as:
  $$
  \begin{aligned}
  CI_{1-\alpha } &=\left[ \hat{\beta}_{1}-t_{n-2,1-\alpha /2}\sqrt{\widehat{Var}\left( \hat{\beta}_{1}\right) },\hat{\beta}_{1}+t_{n-2,1-\alpha /2}\sqrt{\widehat{Var}\left( \hat{\beta}_{1}\right) }\right], \\
  &\text{where }\widehat{Var}\left( \hat{\beta}_{1}\right) =\frac{s^{2}}{\sum_{i=1}^{n}\left( X_{i}-\bar{X}\right) ^{2}}.
  \end{aligned}
  $$
