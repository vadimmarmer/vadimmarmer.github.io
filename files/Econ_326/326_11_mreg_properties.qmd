...
- Recall that $\tilde{X}_{1}$ are the residuals from a regression of $X_{1}$ against $X_{2},\ldots ,X_{k}$ and a constant, and therefore $w_{1,i}$ depends only on $\mathbf{X}$.
...
- Consider $\hat{\beta}_{1}$ and $\hat{\beta}_{2}$:
  ...
  - $\tilde{X}_{1}$ are the fitted residuals from the regression of $X_{1}$ against a constant and $X_{2},X_{3},\ldots ,X_{k}$.

  - $\tilde{X}_{2}$ are the fitted residuals from the regression of $X_{2}$ against a constant and $X_{1},X_{3},\ldots ,X_{k}.$
...
- $\hat{\beta}_{0},\hat{\beta}_{1},\ldots ,\hat{\beta}_{k}$ are linear estimators:
  ...
  and $\tilde{X}_{j,i}$ are the residuals from the regression of $X_{j}$ against the rest of the regressors.
...
- Suppose that instead we regress $Y$ against a constant, $X_{1}$ and additional $k-1$ regressors $X_{2},\ldots ,X_{k}$, i.e., we estimate $\beta _{1}$ by
...
- $\tilde{X}_{1,i}$ are the fitted residuals from regressing $X_{1,i}$ against a constant, $X_{2,i},\ldots ,X_{k,i}$:
...
- In $Y_{i}=\hat{\beta}_{0}+\hat{\beta}_{1}X_{1,i}+\hat{\beta}_{2}X_{2,i}+\ldots +\hat{\beta}_{k}X_{k,i}+\hat{U}_{i},$ the variance of the OLS estimator $\hat{\beta}_{1}$ is
  ...
  where $SSR_{1}$ is the residual sum-of-squares from the regression of $X_{1}$ against a constant and the rest of the regressors.
