...
- **Individual fixed effects** give each individual its own dummy, just like time fixed effects give each period its own dummy. The regression is:
  ...
  $$
  Y_{it} = {\color{purple}\alpha_1} \mathbb{1}[i\!=\!1] + \cdots + {\color{purple}\alpha_n} \mathbb{1}[i\!=\!n] + \text{(time and treatment terms)} + U_{it}.
  $$

  Only the dummy for individual $i$ is active, so the intercept is ${\color{purple}\alpha_i}$. In compact notation:

  $$
  Y_{it} = {\color{purple}\alpha_i} + \sum_{s \neq 0} {\color{blue}\delta_s} \cdot \mathbb{1}[t = s] + {\color{purple}\gamma} D_i + \sum_{s \neq 0} {\color{teal}\beta_s} \cdot D_i \cdot \mathbb{1}[t = s] + U_{it}.
  $$

- But can we include both ${\color{purple}\alpha_i}$ and ${\color{purple}\gamma} D_i$? **No.** $D_i$ does **not change over time** for any individual, so it is already captured by ${\color{purple}\alpha_i}$. Including both would be **perfect multicollinearity**. Drop ${\color{purple}\gamma} D_i$:
...
- Combining individual and time fixed effects gives the **two-way fixed effects** (TWFE) model:
  ...
  $$
  Y_{it} = {\color{purple}\alpha_i} + {\color{blue}\delta_t} + \sum_{s \neq 0} {\color{teal}\beta_s} \cdot D_i \cdot \mathbb{1}[t = s] + U_{it},
  $$

  where ${\color{purple}\alpha_i}$ is the individual fixed effect and ${\color{blue}\delta_t}$ is the time fixed effect.
