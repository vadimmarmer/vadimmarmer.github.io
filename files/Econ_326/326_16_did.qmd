...
- Omitting age biases ${\color{blue}\delta}$, which biases ${\color{teal}\beta}$.
...
- Bias must come from ${\color{blue}\text{difference in trends}}$: a **parallel trends** violation.
...
- With **multiple pre-treatment periods**, use an **event study** design. Data span $t = -T, \ldots, -1, 0, 1, \ldots, T'$, treatment at $t = 0$. Replace the single interaction $\beta(t \cdot D_i)$ with a separate coefficient per period:
...
- ${\color{blue}\delta_s}$: time effect in period $s$ relative to $t = 0$ (generalizes ${\color{blue}\delta}$). ${\color{teal}\beta_s}$: treatment-control difference in period $s$, beyond ${\color{purple}\gamma}$ and ${\color{blue}\delta_s}$.
