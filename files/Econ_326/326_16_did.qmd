...
- At each $t$, exactly one time dummy survives (and at $t = 0$ none do). So $\alpha + {\color{blue}\delta_t}$ is the intercept at time $t$. Define ${\color{blue}\lambda_t} = \alpha + {\color{blue}\delta_t}$ (with $\delta_0 = 0$):
  ...
  The ${\color{blue}\lambda_t}$'s are called **time fixed effects**. The regression with all the $\lambda$ dummies written out:
...
- At each $t$, exactly one $\lambda$-dummy equals $1$ and all others are zero. So the intercept at time $t$ is simply ${\color{blue}\lambda_t}$. In compact **fixed effects notation**:
