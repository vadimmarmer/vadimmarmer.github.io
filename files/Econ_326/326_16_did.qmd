...
- With **multiple pre-treatment periods**, an **event study** design can help. Suppose data span periods $t = -T, \ldots, -1, 0, 1, \ldots, T'$, with treatment at $t = 0$. Extend the DID regression by replacing the single interaction $\beta(t \cdot D_i)$ with a separate coefficient for each period:
  ...
  $$
  Y_{it} = \alpha + \sum_{s \neq 0} {\color{blue}\delta_s} \cdot \mathbb{1}[t = s] + {\color{purple}\gamma} D_i + \sum_{s \neq 0} {\color{teal}\beta_s} \cdot D_i \cdot \mathbb{1}[t = s] + U_{it}.
  $$
...
- With multiple periods $t = 0, 1, 2, \ldots$, we cannot use ${\color{blue}\delta} \cdot t$ because that forces a **linear** trend. Instead, include a separate dummy for each period. The regression actually estimated is:
  ...
  $$
  Y_{it} = \alpha + {\color{blue}\delta_1} \cdot \mathbb{1}[t = 1] + {\color{blue}\delta_2} \cdot \mathbb{1}[t = 2] + \cdots + {\color{purple}\gamma} D_i + \cdots + U_{it}.
  $$

- The key property of the dummies: $\mathbb{1}[t = s]$ equals $1$ when $t = s$ and $0$ otherwise. So at any given time $t$, **all dummies are zero except one**. The intercept becomes:
...
- Each period gets its own intercept. These are called **time fixed effects**. The compact notation writes them as $\sum_{s \neq 0} {\color{blue}\delta_s} \cdot \mathbb{1}[t = s]$. The event study model with time fixed effects:
...
- **Individual fixed effects** give each individual its own dummy, just like time fixed effects give each period its own dummy. The regression actually estimated is:

  $$
  Y_{it} = {\color{purple}\alpha_1} \cdot \mathbb{1}[i = 1] + {\color{purple}\alpha_2} \cdot \mathbb{1}[i = 2] + \cdots + {\color{purple}\alpha_n} \cdot \mathbb{1}[i = n] + \text{(time and treatment terms)} + U_{it}.
  $$

  At any given observation, only the dummy for individual $i$ is active, so the intercept is ${\color{purple}\alpha_i}$.

## Why $D_i$ is absorbed

- With individual fixed effects, can we still include ${\color{purple}\gamma} D_i$ in the regression?

- **No.** $D_i$ is a characteristic of individual $i$ that does **not change over time**: a house is either near the incinerator or it is not, in every period. So $D_i$ is already fully captured by ${\color{purple}\alpha_i}$.

- Formally, $D_i$ is a linear combination of the individual dummies (it equals $1$ for some individuals and $0$ for others). Including both ${\color{purple}\alpha_i}$ and ${\color{purple}\gamma} D_i$ would be **perfect multicollinearity**.

- The model with both individual and time fixed effects:

  $$
  Y_{it} = {\color{purple}\alpha_i} + \sum_{s \neq 0} {\color{blue}\delta_s} \cdot \mathbb{1}[t = s] + \sum_{s \neq 0} {\color{teal}\beta_s} \cdot D_i \cdot \mathbb{1}[t = s] + U_{it}.
  $$
