...
- The classic DID diagram shows the control group, treatment group, and counterfactual:
  ...
  # delta: time effect (control group change from t=0 to t=1)
  # reference line at baseline alpha from t=0 to t=1
  segments(0, alpha, 1, alpha, lty = 3, col = "blue", lwd = 0.8)
  arrows(0.95, alpha, 0.95, alpha + delta,
         code = 3, length = 0.08, lwd = 1, col = "blue")
  text(0.85, (alpha + alpha + delta) / 2, expression(delta), cex = 1,
       col = "blue", pos = 2)
  ```
...
- The pre-treatment coefficients $\mu_k$ for $k < 0$ serve as a diagnostic:
  ...
  |---|---|
  | All close to zero: $\mu_{-3} \approx \mu_{-2} \approx \mu_{-1} \approx 0$ | Both assumptions look plausible |
  | Sharp break right before treatment: $\mu_{-3} \approx \mu_{-2} \approx 0$ but $\mu_{-1} \neq 0$ | **Anticipation**: units react to news of upcoming treatment |
  | Gradual trend across pre-periods: $|\mu_{-3}| < |\mu_{-2}| < |\mu_{-1}|$, growing steadily | **Parallel trends violation**, not anticipation: groups were already diverging before treatment |
