---
# Rendered files go to output/, set by `output-dir` in _quarto.yml.
# Quarto has no document-level setting for this: it finds _quarto.yml by
# walking up from this file, so the destination holds wherever the render
# is started from. Both HTML formats below must stay self-contained.
...
- $\rank{X^\top X}= \rank{X}$.
- $(X^\top X)^{-1}$ exists exactly when $\rank{X} = k$, meaning the columns of $X$ are linearly independent:
$$
