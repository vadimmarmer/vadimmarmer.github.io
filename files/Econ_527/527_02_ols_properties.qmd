...
- The numbering is the typed notes'. Assumption 4 is the sample rank condition, the sample counterpart of $\operatorname{rank}\big(\E{X_i X_i^\top}\big) = k$ from the first lecture.
...
- The law of iterated expectations, from the first lecture, finishes the proof whenever $\E{\hat\beta}$ exists:
$$
...
- Conditioning on $X_1, \ldots, X_n$ makes the denominator and every $X_i$ in the numerator known:
$$
...
- Take one regressor, so $X_i$ is a scalar with density $f_X$, and write $g(x) = \E{U_i \mid X_i = x}$. By iterated expectations,
$$
\E{X_i U_i} = \E{\E{X_i U_i \mid X_i}} = \E{X_i\, g(X_i)} = \int x\, g(x) f_X(x)\, dx.
$$
...
$$
\int x\, g(x) f_X(x)\, dx = 0 \;\text{ does not imply }\; g(x) = 0 \ \text{ for all } x.
$$

## The parabola

- Let the conditional mean of the error be a parabola in the regressor, centred at zero and shifted down so that $\E{U_i} = \E{g(X_i)} = 0$, and let the regressor be symmetric about zero. Then $g$ is even, so $x\, g(x)$ is odd and $\E{X_i U_i} = 0$, while $\E{U_i \mid X_i} \neq 0$ at every value of $X_i$ except the two zeros of $g$.
...
- Read the horizontal axis as education and the vertical axis as ability: ability is low at middling education and high at both ends.
  ...
  #| fig-height: 3.6
  #| fig-dpi: 300
  tikz_fig("parabola", 9, 3.6, {
    set.seed(2)
    op <- par(mfrow = c(1, 2), mar = c(3.8, 2.4, 2.2, 0.6), font.main = 1)
    xg <- seq(-2.5, 2.5, length.out = 400)
    xs <- runif(140, -2.5, 2.5)
    panel <- function(u_values, main_tex, draw_curve) {
      plot(NA, xlim = c(-2.5, 2.5), ylim = c(-3, 6), xlab = "", ylab = "",
           axes = FALSE, main = main_tex)
      box()
      mtext("$x$", side = 1, line = 1.1)
      mtext("$u$", side = 2, line = 1.0)
      mtext("$\\mathrm{E}[X_i U_i] = 0$", side = 1, line = 2.9)
      points(xs, u_values, pch = 19, cex = 0.5, col = "gray60")
      abline(h = 0, lwd = 2, col = "#d95f02")
      if (draw_curve) lines(xg, xg^2 - 25/12, lwd = 2, col = "#1b9e77")
    }
    panel(xs^2 - 25/12 + rnorm(140, sd = 0.7),
          "$\\mathrm{E}[U_i \\mid X_i] \\neq 0$", TRUE)
    panel(rnorm(140, sd = 0.7),
          "$\\mathrm{E}[U_i \\mid X_i] = 0$", FALSE)
    par(op)
  })
  ```
...
- Under $\E{X_i U_i} = 0$ and $\E{U_i} = 0$ in place of Assumption 2: $\beta$ is identified, and $\hat\beta$ is still the method-of-moments and least-squares estimator.
