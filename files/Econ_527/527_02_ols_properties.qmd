...
$$
OLS is the case $W = I_n$, but for other choices of $W$ the estimator is different.

## Both are unbiased

- The two-point slope, assume $Y_i = \beta X_i + U_i$ and $\E{U_i \mid X} = 0$:
$$
\tilde\beta = \beta + \frac{U_n - U_1}{X_n - X_1}, \qquad \E{\tilde\beta \mid X} = \beta.
$$

- Weighted least squares, assume $Y = X\beta + U$ and $\E{U \mid X} = 0$:
$$
...
- One can show that under Assumptions 1-4,
$$
...
- Suppose $M = \Var{\tilde\beta \mid X} - \Var{\hat\beta \mid X}\ge 0$ in the positive semidefinite sense. Then:
$$
...
- **Theorem.** Suppose Assumptions 1 to 4 hold:
  1. $Y = X\beta + U$ for some $\beta \in \mathbb{R}^k$;
  2. $\E{U \mid X} = 0$;
  3. $\Var{U \mid X} = \sigma^2 I_n$ for some $\sigma^2 > 0$;
  4. $\operatorname{rank}(X) = k$.
...
- Let $\tilde\beta$ be any estimator that is
  ...
  - **conditionally unbiased**: $\E{\tilde\beta \mid X} = \beta$ for every value of $\beta$.

- Then
$$
\Var{\tilde\beta \mid X} - \Var{\hat\beta \mid X}\ge 0 \ \text{(positive semidefinite).}
$$
...
- This must hold for every value of $\beta$ and all realizations of $X$, so
$$
\boxed{\ A X = I_k\ }
$$
...
- In our examples:
  - OLS: $A = (X^\top X)^{-1} X^\top$, so: 
  $$
  A X = (X^\top X)^{-1} X^\top X = I_k.
  $$
...
- Using $A X = I_k$ for the general estimator:
$$
...
- For OLS: 
  $$
  \hat\beta - \beta = (X^\top X)^{-1} X^\top U.
  $$

- Consider the covariance between $\tilde\beta$ and $\hat\beta$:
$$
\begin{aligned}
\Cov{\tilde\beta, \hat\beta \mid X}
&= \E{\big(\tilde\beta - \beta\big)\big(\hat\beta - \beta\big)^\top \,\Big|\, X} \\
&= \E{\underbrace{A U}_{\tilde\beta - \beta}\, \underbrace{U^\top X (X^\top X)^{-1}}_{(\hat\beta - \beta)^\top} \,\big|\, X} \\
...
- Expand the variance of the difference:
$$
\begin{aligned}
\Var{\tilde\beta - \hat\beta \mid X} &= \Var{\tilde\beta \mid X} + {\color{blue}\Var{\hat\beta \mid X}} \\
&\quad- {\color{blue}\Cov{\tilde\beta, \hat\beta \mid X}} - {\color{blue}\Cov{\hat\beta, \tilde\beta \mid X}}\\
&= \Var{\tilde\beta \mid X} + {\color{blue}\Var{\hat\beta \mid X}} \\
&\quad - \underbrace{\color{blue}\Var{\hat\beta \mid X}}_{=\Cov{\tilde\beta, \hat\beta \mid X}} - \underbrace{\color{blue}\Var{\hat\beta \mid X}}_{=\Cov{\hat\beta, \tilde\beta \mid X}^\top} \\
...
- Therefore:
$$
0\leq \Var{\tilde\beta - \hat\beta \mid X} = \Var{\tilde\beta \mid X} - \Var{\hat\beta \mid X}.
$$

## Uniqueness

- Suppose some linear conditionally unbiased $\tilde\beta$ attains the same variance matrix. By the identity of step 3,
$$
\Var{\tilde\beta \mid X} = \Var{\hat\beta \mid X}
...
$$
\tilde\beta - \hat\beta = \delta(X)
$$
for some function of $X$ alone.

- Both are conditionally unbiased, so taking conditional expectations,
$$
\underbrace{\E{\tilde\beta \mid X}}_{=\,\beta} = \underbrace{\E{\hat\beta \mid X}}_{=\,\beta} + \delta(X)
\quad\Longrightarrow\quad \delta(X) = 0.
$$
...
- Assumption 3 entered in step 2, where $\E{U U^\top \mid X}$ was replaced by $\sigma^2 I_n$, and again when $\sigma^2 (X^\top X)^{-1}$ was recognised as $\Var{\hat\beta \mid X}$. Without it the cancellation that produced $\sigma^2 A X$ fails, and the covariance need not equal the variance of OLS.
...
&Y^* = X^* \beta + U^*,\\
& \E{U^* \mid X} = 0,\\
& \Var{U^* \mid X} = \Omega^{-1/2} \Omega \Omega^{-1/2} = I_n.
\end{align*}


- The transformed model satisfies all four assumptions, so Gauss-Markov applies to it.
- OLS on the transformed model is the best linear unbiased estimator:
$$
\hat\beta^* = (X^{*\top} X^*)^{-1} X^{*\top} Y^* = (X^\top \Omega^{-1} X)^{-1} X^\top \Omega^{-1} Y,
$$
which is weighted least squares with $W = \Omega^{-1}$. This is the **GLS estimator**: generalized least squares.

- Suppose $\E{U_i^2 \mid X} = \sigma_i^2$ and $\E{U_i U_j \mid X} = 0$ for $i \neq j$, so that
  $$
  \Omega = \begin{pmatrix} \sigma_1^2 & & 0 \\ & \ddots & \\ 0 & & \sigma_n^2 \end{pmatrix}.
  $$
- Then the GLS estimator is:
  $$
  \hat\beta^* = (X^\top \Omega^{-1} X)^{-1} X^\top \Omega^{-1} Y = \left(\sum_{i=1}^n \frac{X_i X_i^\top}{\sigma_i^2}\right)^{-1} \sum_{i=1}^n \frac{X_i Y_i}{\sigma_i^2}.
  $$
  Observations with larger $\sigma_i^2$ get less weight, and those with smaller $\sigma_i^2$ get more weight.

- This is equivalent transforming the data by scaling each observation $i$ by its $\sigma_i$:

  $$
  \frac{Y_i}{\sigma_i} = \frac{X_i^\top}{\sigma_i} \beta + \frac{U_i}{\sigma_i}, \qquad i = 1, \ldots, n,
  $$
  so that $\Var{U_i / \sigma_i \mid X} = 1$ for every $i$.
