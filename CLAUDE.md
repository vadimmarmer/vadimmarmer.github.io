...
- **Every HTML file that is committed must carry everything it needs inside it**, with images, styles, fonts and scripts embedded, which `embed-resources: true` does. In `files/Econ_527/` this is set once in `_quarto.yml`, for the page format and the slide show format, so every deck there inherits it; do not set it in a deck. The Econ 326 decks set it in each file's frontmatter instead, and eleven of the twenty do not set it at all, which is why `files/Econ_326/output/` carries thirteen `326_XX_files/` folders of supporting libraries. Those folders are committed, so those pages work, but a page whose folder went uncommitted would break as soon as it was published.
