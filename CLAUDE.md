...
- **Every HTML file that is committed must carry everything it needs inside it**, with images, styles, fonts and scripts embedded, which `embed-resources: true` does. In `files/Econ_527/` this is set once in `_quarto.yml`, for the page format and the slide show format, so every deck there inherits it; do not set it in a deck. The Econ 326 decks set it in each file's frontmatter instead.
